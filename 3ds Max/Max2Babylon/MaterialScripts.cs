...
namespace Max2Babylon
{
    ...
    {
        public static string AsInlineScript( this string src)
        {
            return src.Replace("\"", "\\\"");
        }

        public static string AddCustomAttribute( this string decl, string container, string def)
        {
            return $@"MATERIAL_CAT_DEF = {decl}
                   maxMaterial = sceneMaterials[""{container}""]
                   custAttributes.add maxMaterial MATERIAL_CAT_DEF;";
        }
        ...
        public static string AddCallback => 
            $@"
            addMaterialCallbackScript = ""maxMaterial = callbacks.notificationParam();

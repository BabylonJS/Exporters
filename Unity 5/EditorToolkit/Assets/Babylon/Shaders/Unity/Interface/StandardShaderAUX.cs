...
{
    public class StandardShaderAUX : ShaderGUI
    {
            ...
            if (props != null && props.Length > 0) {
                foreach (MaterialProperty prop in props) {
                    if (Unity3D2Babylon.SceneBuilder.SystemPredefinedPropertyNames.IndexOf(prop.name, StringComparison.OrdinalIgnoreCase) == -1) {
                        //UnityEngine.Debug.Log("===> Babylon Render Property: " + prop.name);
                        customProps.Add(prop);
                    }
                }
            }
            // Babylon Render Options
            m_ShowBabylon = EditorGUILayout.Foldout(m_ShowBabylon, "Babylon Rendering");
            if (m_ShowBabylon)
            {
                ...
                if (customProps.Count > 0) {
                    for (var i = 0; i < customProps.Count; i++) {
                        if ((customProps[i].flags & (MaterialProperty.PropFlags.HideInInspector | MaterialProperty.PropFlags.PerRendererData)) != 0)
                            continue;
                        materialEditor.ShaderProperty(customProps[i], customProps[i].displayName);
                    }
                }
                EditorGUI.indentLevel -= 1;
            }
        ...
        void DoNormalArea()
        {
            ...
            if (bumpScale.floatValue != 1 && UnityEditorInternal.InternalEditorUtility.IsMobilePlatform(EditorUserBuildSettings.activeBuildTarget)) {
                var messageContent = new GUIContent("Bump scale is not supported on mobile platforms", "Bump scale is not supported on mobile platforms");
                var buttonContent = new GUIContent("Fix Now", "Fix Now");
                if (m_MaterialEditor.HelpBoxWithButton(messageContent, buttonContent))
                {

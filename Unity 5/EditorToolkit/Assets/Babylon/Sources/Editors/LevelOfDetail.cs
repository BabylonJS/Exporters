...
namespace UnityEditor
{
    ...
    public sealed class LevelOfDetail : EditorScriptComponent
    {
        ...
        public bool autoGenerateLevels = false;

        [Header("-Distance Properties-"), Unity3D2Babylon.ReadOnly]
        
        [Range(0.1f, 1.0f)]
        public float cameraDistanceFactor = ExporterWindow.exportationOptions.CameraDistanceFactor;
    ...
    public class LevelDistanceEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var levelDetail = serializedObject.FindProperty("levelDetailGroup");
            if (levelDetail != null) LevelDistanceEditorList.Show(levelDetail, this);
            
            var autoGenerate = serializedObject.FindProperty("autoGenerateLevels");
            if (autoGenerate != null) LevelDistanceEditorList.Show(autoGenerate, this);
            
            var cameraDistance = serializedObject.FindProperty("cameraDistanceFactor");
            if (cameraDistance != null) LevelDistanceEditorList.Show(cameraDistance, this);
            
            var groupLevel = serializedObject.FindProperty("groupLevelDistances");
            if (groupLevel != null) LevelDistanceEditorList.Show(groupLevel, this);

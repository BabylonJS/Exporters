...
namespace BabylonHosting
{
    public static class WebServer
    {
        ...
        static WebServer()
        {
            WebServer.Root = String.Empty;
            WebServer.Unity = String.Empty;
            WebServer.IsStarted = false;
            WebServer.IsSupported = false;
            WebServer.HostListener = null;
            WebServer.SetupFileTypeMappings();
        }
        ...
        public static bool Activate(string prefix, string root, int port, string unity)
        {
            ...
            if (WebServer.HostListener == null)
            {
                try
                {
                    ...
                    WebServer.HostListener.BeginGetContext(WebServer.ClientConnected, null);
                    // ..
                    WebServer.IsStarted = true;
                    WebServer.IsSupported = true;
                }
                catch
                {
                    WebServer.HostListener = null;
        ...
        static void ClientConnected(IAsyncResult state)
        {        
            HttpListenerContext clientContext = WebServer.HostListener.EndGetContext(state);
            WebServer.HostListener.BeginGetContext(WebServer.ClientConnected, null);
            // ..
            WebServer.ProcessRequest(clientContext);
        }
        ...
        static void WriteResponse(HttpListenerContext context, FileInfo path)
        {
            ...
            WebServer.WriteResponse(context, file);
            file.Close();
        }
